...
Page({
    ...
    sendAudioMsg(res) {
        ...
        var l = {
            users: {
                "receive": {
                    "nick": app.globalData.zhubo.nick,
                    "portrait": app.globalData.zhubo.portrait,
                    "uid": '215213'
                },
                "sender": {
                    "nick": app.globalData.userInfo.nick,
                    "portrait": app.globalData.userInfo.portrait,
...
// function getDetail(self,callback) {
...
//     }
//     // app.globalData.chatAllList.push(app.globalData.zhubo)
//     // console.log(app.globalData.chatAllList)
//     if (utils.isFunction(callback)){
//       callback(self)
//     }
//   })
// }
function loadNode(xx) {
    var appInfo = app.globalData.userInfo;
    var zhuboInfo = app.globalData.zhubo;
    ...
    if (chatTo === appInfo['uid']) {
        ...
        this.setData({
            chatTo: zhuboInfo['uid'],
            loginAccountLogo: appInfo['portrait'] || this.data.defaultUserLogo,
            chatToLogo: zhuboInfo['portrait'] || this.data.defaultUserLogo,
        })
    } else {
        // 设置头像数据
        console.log(zhuboInfo)
        app.globalData.nim.getUser({
            account: chatTo,
                ...
                wx.setNavigationBarTitle({
                    title: zhuboInfo.nick,
                })

                let chatToLogo = ''
                if (zhuboInfo['portrait']) {
                    chatToLogo = zhuboInfo['portrait']
                } else {
                    chatToLogo = self.data.defaultUserLogo
                }
                self.setData({
                    chatToLogo,
                    loginAccountLogo: appInfo['portrait'] || self.data.defaultUserLogo
                })
            }
        })
    }
    ...
    // 渲染应用期间历史消息
    let loginUserAccount = appInfo['uid']
    let loginMessageList = app.globalData.messageList[loginUserAccount]
