    ...
    methods: {
      ...
      initYScale() {
        ...
        this.yScale = d3.scaleLinear()
          ...
          .domain([0, d3.max(this.dataList.map((d) => d.value + 30))]);
      },
      drawChart() {
        let that = this;
        ...
        this.group.selectAll('.areaCircle')
          .data(this.list)
          .enter()
          .append('circle')
        ...
        this.group.selectAll('.areaOuterCircle')
          .data(this.list)
          .enter()
          .append('circle')
...
//          item.x = this.xScale(item.name);
//          item.y = this.yScale(item.value);
//        });
        //利用clipPath方法做动画渲染
        let area = d3.area()
          .x((d) => this.xScale(d.name))
          .y1((d) => this.yScale(d.value));
        //  .curve(d3.curveCatmullRom.alpha(1));//如果需要设置曲线放开代码即可.
        let linearGradientId = new Date().getTime();
        this.svg.append('defs').html(
          '<linearGradient id=' + linearGradientId + ' x1="0%" x2="0%" y1="0%" y2="100%">\n' +
          '        <stop offset="0%" stop-color=' + this.colorList[0] + '></stop>\n' +
          '        <stop offset="100%" stop-color=' + this.colorList[0] + ' stop-opacity="0"></stop>\n' +
          '      </linearGradient>');
        ...
        // .curve(d3.curveCatmullRom.alpha(1));//如果需要设置曲线,放开代码即可
        this.group.append('path')
          .datum(this.list)
          .attr("fill", "none")
      ...
      drawLegend() {
        ...
        textList.attr('transform', function (d) {
          return "translate(" + [d.left, d.lineIndex * 20] + ")"
        })
      },
      calculateWidth(str) {
      ...
      drawAxis() {
        ...
        if (this.axisShow) {
          ...
          this.yAxis.select('path')
            .remove();
          this.yAxis.selectAll('line').attr('class', 'gridLine').attr('opacity', 0);

          //添加x轴主线
          this.xAxis.append('line')
            .attr('x1', 0)
            .attr('y1', 0)
            .attr('x2', this.boxWidth - 10)
            .attr('y2', 0)
            .style('stroke', '#9b9b9b')
            .attr('stroke-width', '1');
          //添加y轴主线
...
//        });
        ...
        let area = d3.area()
          .x((d) => this.xScale(d.name))
          .y1((d) => this.yScale(d.value));
        let linearGradientId = Math.random() * 10000;
        //  .curve(d3.curveCatmullRom.alpha(1));//如果需要设置曲线放开代码即可.
        this.svg.append('defs').html('<linearGradient id=' + linearGradientId + ' x1="0%" x2="0%" y1="0%" y2="100%">\n' +
          '        <stop offset="0%" stop-color=' + this.colorList[0] + '></stop>\n' +
          '        <stop offset="100%" stop-color=' + this.colorList[0] + ' stop-opacity="0"></stop>\n' +
          '      </linearGradient>');
        area.y0(this.yScale(0));

        let areaClipId = new Date().getTime();
        this.group.append('path')
          .datum(this.dataList)
          .attr("fill", "url(#" + linearGradientId + ")")
          .attr("d", area)
          .attr('clip-path', 'url(#' + areaClipId + ')');
    ...
    watch: {
      json: {
        handler(old, val) {
          this.init();
          if (old && val) {
            if (JSON.stringify(old.width) !== JSON.stringify(val.width)
              || JSON.stringify(old) === JSON.stringify(val)) {
              return
            } else {
              this.drawPortion();
              setTimeout(() => {
