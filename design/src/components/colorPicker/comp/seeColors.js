...
class SeeColors {
	...
	constructor(imgUrl,option,fc) {
		...
		//
		// 	let {width,height}=await getImgWH(imgUrl)
		//
		// 	this.option = option || {
		// 		width: width,
		// 		height: height,
	...
	// 	let img=new Image
	// 	w.style.cssText='position:fixedtop:0left:0width:100%height:100%background:rgba(0,0,0,.24)z-index:9999'
	// 	img.src='./onloadMaterial.gif'
	// 	img.style.cssText='position:absolutetop:50%left:50%transform:translate(-50%,-50%)'
	// 	w.appendChild(img)
	// 	return w
	// }
		...
		return this.createImage().then(function (can) {
			...
			return this.addListener(this.$('.seeColors-temp-container'),
				(x, y) => {
					...
					if(x<react.left||y<react.top||x>react.left+react.width||y>react.top+react.height){
	...
	createImage() {
		...
		return new Promise((resolve, reject) => {
			let canvas = document.createElement('canvas'),{width,height}=this.option
			// canvg(c, this.svg, {
